...
class MyFrame(wx.Frame):
	def __init__(self):
		...
		self.converButton = wx.Button(panel, -1, "Convert it!")
		self.Bind(wx.EVT_BUTTON, self.OnClickConverButton, self.converButton)

		sizer.AddMany([
			wx.StaticText(panel, size=(20, 10)),
			wx.StaticText(panel),
			wx.StaticText(panel),
			wx.StaticText(panel),
			wx.StaticText(panel),
	...
	def OnClickBrowseButton(self, event):
		wildcard = "Source file (*dat)|*data|All files (*.*)|*.*"
	...
	def OnClickConverButton(self, event):
